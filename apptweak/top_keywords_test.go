...
func TestTopKeywords(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			token:         "12345x",
			appID:         1414415906,
			urlPath:       "/ios/applications/1414415906/keywords/top.json",
			options:       Options{},
			expectedError: nil,
			responseCode:  200,
			fixture:       "./fixtures/top_keywords.json",
			firstKeyword:  "udemy",
		},
	}
	...
	for _, tc := range tests {
		t.Run(tc.description, func(t *testing.T) {
			...
			resp, err := client.TopKeywords(tc.appID, tc.options)
			...
			assert.Equal(t, tc.firstKeyword, resp.KeywordList[0].Keyword, tc.description)
